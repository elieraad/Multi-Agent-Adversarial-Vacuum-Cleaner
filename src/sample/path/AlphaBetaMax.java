...
public class AlphaBetaMax extends PathFinder {

    public static Tuple run(int[][] map, List<Tuple> agent, Tuple dirtAgent, int nbRows, int nbColumns, int depth, int alpha, int beta, boolean maximizingPlayer) {
...
//        if (maximizingPlayer) {
        ...
        for (Tuple move : getChildren(backgroundMap, nbRows, nbColumns, dirtAgent)) {
            ...
            int eval = max(backgroundMap, agent, dirtAgent, nbRows, nbColumns, depth, alpha, beta);
...
//        }
    ...
    public static int max(int[][] map, List<Tuple> agent, Tuple dirtAgent, int nbRows, int nbColumns, int depth, int alpha, int beta) {
        if (depth == 0) {
        ...
        for (Tuple move : getChildren(map, nbRows, nbColumns, dirtAgent)) {
            ...
            map[dirtAgent.row][dirtAgent.col] = Main.empty;
            map[move.row][move.col] = Main.dirt_agent;

            int eval = min(map, agent, move, nbRows, nbColumns, depth - 1, alpha, beta);
    ...
    public static int min(int[][] map, List<Tuple> agentList, Tuple dirtAgent, int nbRows, int nbColumns, int depth, int alpha, int beta) {
        if (depth == 0)
            return getDirtTilesFullyObs(map, nbRows, nbColumns).size();


        int minEval = Integer.MAX_VALUE;
        for (Tuple agent : agentList) {
            ...
            int eval = max(map, moves, dirtAgent, nbRows, nbColumns, depth - 1, alpha, beta);
