...
public class AlphaBetaMin extends PathFinder {

    public static Tuple run(int[][] map, Tuple agent, List<Tuple> dirtAgent, int nbRows, int nbColumns, int depth, int alpha, int beta, boolean maximizingPlayer) {
...
//        }
    ...
    public static int max(int[][] map, Tuple agent, List<Tuple> dirtAgentList, int nbRows, int nbColumns, int depth, int alpha, int beta) {
        if (depth == 0) {
        ...
        for (Tuple dirtAgent : dirtAgentList) {
            ...
            int eval = min(map, agent, moves, nbRows, nbColumns, depth - 1, alpha, beta);
    ...
    public static int min(int[][] map, Tuple agent, List<Tuple> dirtAgentList, int nbRows, int nbColumns, int depth, int alpha, int beta) {
        if (depth == 0)
            return getDirtTilesFullyObs(map, nbRows, nbColumns).size();
        ...
        for (Tuple move : getChildren(map, nbRows, nbColumns, agent)) {
            ...
            int eval = max(map, move, dirtAgentList, nbRows, nbColumns, depth - 1, alpha, beta);
