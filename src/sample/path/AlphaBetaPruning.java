...
public class AlphaBetaPruning extends PathFinder {

    public static Tuple run(int[][] map, Tuple agent, Tuple dirtAgent, int nbRows, int nbColumns, int depth, int alpha, int beta, boolean maximizingPlayer) {
        ...
        if (maximizingPlayer) {
            ...
            for (Tuple move : getChildren(backgroundMap, nbRows, nbColumns, agent)) {
                ...
                int eval = min(backgroundMap, agent, dirtAgent, nbRows, nbColumns, depth, alpha, beta);
                System.out.printf("Current move: %s, Current eval %s, Minimum Eval: %s%n", move, eval, minEval);
    ...
    public static int max(int[][] map, Tuple agent, Tuple dirtAgent, int nbRows, int nbColumns, int depth, int alpha, int beta) {
        if (depth == 0)
    ...
    public static int min(int[][] map, Tuple agent, Tuple dirtAgent, int nbRows, int nbColumns, int depth, int alpha, int beta) {
        if (depth == 0)
            return getDirtTilesFullyObs(map, nbRows, nbColumns).size();
        ...
        for (Tuple move : getChildren(map, nbRows, nbColumns, agent)) {
            ...
            int eval = max(map, move, dirtAgent, nbRows, nbColumns, depth - 1, alpha, beta);
