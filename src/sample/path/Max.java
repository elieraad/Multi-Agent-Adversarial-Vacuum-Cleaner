...
public class Max extends PathFinder {

    public static Tuple run(int[][] map, List<Tuple> agent, Tuple dirtAgent, int nbRows, int nbColumns, int depth, boolean maximizingPlayer) {
...
//        }
    ...
    public static int max(int[][] map, List<Tuple> agent, Tuple dirtAgent, int nbRows, int nbColumns, int depth) {
        if (depth == 0) {
    ...
    public static int min(int[][] map, List<Tuple> agentList, Tuple dirtAgent, int nbRows, int nbColumns, int depth) {
        if (depth == 0)
            return getDirtTilesFullyObs(map, nbRows, nbColumns).size();
        ...
        for (Tuple agent : agentList) {
            ...
            int eval = max(map, moves, dirtAgent, nbRows, nbColumns, depth - 1);
